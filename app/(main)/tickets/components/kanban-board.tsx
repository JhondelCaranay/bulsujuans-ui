...
export function KanbanBoard() {
  ...
  return (
    <div className="h-full w-full flex flex-col overflow-auto bg-gradient-to-br from-slate-50 via-white to-slate-60 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 p-8">
      ...
      {/* Kanban Grid */}
      <div className="flex gap-6 p-4 overflow-auto ">
        {COLUMNS.map((column) => {
          const columnTickets = tickets.filter((t) => t.status === column);
          const colors = ticketStatusConfig[column];

          return (
            <div key={column} className="flex flex-col min-w-[350px] max-w-[350px] flex-shrink-0">
              <div className="mb-4">
